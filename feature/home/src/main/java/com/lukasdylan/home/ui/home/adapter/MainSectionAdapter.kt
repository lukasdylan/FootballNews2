...
class MainSectionAdapter(layoutMaxWidth: Int, private val listener: (NavigationScreen) -> Unit) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            HomeSectionType.TRENDING_LEAGUE_NEWS.ordinal -> {
                ...
                TrendingNewsSectionViewHolder(binding, trendingNewsSectionAdapter, leagueName, pool)
            }
            HomeSectionType.STANDINGS.ordinal -> {
                ...
                StandingsSectionViewHolder(binding, standingsSectionAdapter, listener, pool)
            }
            HomeSectionType.NEXT_MATCH.ordinal -> {
                ...
                NextMatchSectionViewHolder(binding, nextMatchSectionAdapter, listener, pool)
            }
            HomeSectionType.PREV_MATCH.ordinal -> {
                ...
                PreviousMatchSectionViewHolder(binding, previousMatchSectionAdapter, listener, pool)
            }
    ...
    class StandingsSectionViewHolder(
        ...
        fun bind(data: List<Standings>, isLoading: Boolean) {
            with(binding) {
                tvSeeAllStandings.visibility = if (data.isEmpty()) View.GONE else View.VISIBLE
    ...
    class PreviousMatchSectionViewHolder(
        ...
        fun bind(data: List<DetailMatch>, isLoading: Boolean) {
            with(binding) {
                tvSeeAllMatch.visibility = if (data.isEmpty()) View.GONE else View.VISIBLE
                shimmerLayout.onAnimateListener(isLoading)
